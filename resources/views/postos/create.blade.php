<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-center text-gray-800 leading-tight">
            Novo registro de posto
        </h2>
    </x-slot>
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('postos.store') }}">
                    @csrf
                    <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="cidade_id" class="block font-medium text-sm text-gray-700">Cidade (ID)</label>
                            <input type="number" name="cidade_id" id="cidade_id" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                value="{{ old('cidade_id', '') }}" />
                            @error('cidade_id')
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="number" name="reservatorio" id="reservatorio" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                value="{{ old('reservatorio', '') }}" min='0' max='100' step='0.1'/>
                            @error('reservatorio')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
