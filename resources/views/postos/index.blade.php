<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-center text-gray-800 leading-tight">
            {{ __('Lista de postos registradas') }}
        </h2>
    </x-slot>
    <div class="py-12">
                    ...
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 ">
                                    @foreach ($postos as $posto)
                                        <tr>
                                            ...
                                            <td class="flex-grow px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ $posto->cidade->nome_da_cidade }} ({{ $posto->cidade->id }})
                                            </td>
                                            <td class="flex flex-grow px-6 py-4 whitespace-nowrap text-sm font-medium gap-1">
                                                <a href="{{ route('postos.show', $posto->id) }}" class="flex-grow items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                                                    Ver
                                                </a>
                                                <a href="{{ route('postos.edit', $posto->id) }}" class="flex-grow items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                                                    Editar
                                                </a>
                                                <form class="flex " action="{{ route('postos.destroy', $posto->id) }}" method="POST" onsubmit="return confirm('Você tem certeza disso?');">
                            ...
                            <div class="flex items-center px-4 py-3 bg-gray-50 text-right sm:px-6">
                                <a href="{{ route('postos.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                                    Criar novo registro
                                </a>
                            </div>
                        </div>
